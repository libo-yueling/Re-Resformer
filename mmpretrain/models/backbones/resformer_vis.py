...
class LearnableGlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 返回加权后的特征图和用于可视化的掩码
        out = x * attn
        return out,attn_global,defect_mask,attn
...
# 五个部分：DSConv1 -> GN1 -> DSConv2 -> GN2 -> KAN
class ResformerEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.attten1 = LearnableGlobalAttention(hidden_channels)
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            mask: [B, hidden_channels, H', W'] （LearnableGlobalAttention 输出的 mask）
            feat_raw: [B, hidden_channels, H', W'] （Attention 之前的原始特征）
        """
        # 1. 如果需要，先做 1×1 通道对齐
        x = self.proj1(x)  # [B, hidden_channels, H', W']

        # 2. 保存 attention 之前的 raw 特征
        feat_raw = x.clone()  # [B, hidden_channels, H', W']

        # 3. LearnableGlobalAttention
        out_attn, attn_global,defect_mask,attn = self.attten1(x)  # out_attn=[B, hidden_channels, H', W']; mask=[B, hidden_channels, H', W']
        x = out_attn

        # 4. DSConv + GN + DSConv + 残差
        x = self.dsconv1(x)         # [B, hidden_channels, H', W']
        residual = x.clone()        # 保存残差分支
        x = self.gn(x)             # GroupNorm
        x = self.dsconv2(x)         # 第二次 DSConv
        x = x + residual            # 残差连接

        # 5. KAN + proj2(residual)
        residual2 = x.clone()
        x = self.kan(x)             # [B, out_channels, H', W']
        x = x + self.proj2(residual2)  # proj2: [B, hidden_channels, H', W'] -> [B, out_channels, H', W']

        out = x  # [B, out_channels, H', W']
        return out, out_attn,attn_global,defect_mask, feat_raw


class BasicBlock(BaseModule):
    """BasicBlock for ResFormer.
...
class Bottleneck(BaseModule):
    """Bottleneck block for ResFormer.
...
class ResLayer(nn.Sequential):
    """ResLayer to build ResFormer style backbone.
...
class ResFormer(BaseBackbone):
    ...
    Example:
        >>> from mmpretrain.models import ResFormer
        >>> import torch
        >>> self = ResFormer(depth=18)
        >>> self.eval()
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.0):
        super(ResFormer, self).__init__(init_cfg)
        if depth not in self.arch_settings:
            raise KeyError(f'invalid depth {depth} for resformer')
        self.depth = depth
    ...
    def init_weights(self):
        super(ResFormer, self).init_weights()
    ...
    def train(self, mode=True):
        super(ResFormer, self).train(mode)
        self._freeze_stages()
    ...
    def get_layer_depth(self, param_name: str, prefix: str = ''):
        """Get the layer id to set the different learning rates for ResFormer.
...
@MODELS.register_module()
class ResFormerV1c(ResFormer):
    """ResFormerV1c backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default ResFormer(ResFormerV1b), ResFormerV1c replaces the 7x7 conv
    in the input stem with three 3x3 convs.
    """

    def __init__(self, **kwargs):
        super(ResFormerV1c, self).__init__(
            deep_stem=True, avg_down=False, **kwargs)


@MODELS.register_module()
class ResFormerV1d(ResFormer):
    """ResFormerV1d backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default ResFormer(ResFormerV1b), ResFormerV1d replaces the 7x7 conv in
    the input stem with three 3x3 convs. And in the downsampling block, a 2x2
    avg_pool with stride 2 is added before conv, whose stride is changed to 1.
    """

    def __init__(self, **kwargs):
        super(ResFormerV1d, self).__init__(
