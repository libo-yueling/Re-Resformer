...
class HiViT(BaseBackbone):
    ...
    def forward(self, x):
        """
        Args:
            x (tensor): Image tensor of shape (B, C, H, W) where B is batch size,
                        C is number of channels, H is height, and W is width.

        Returns:
            Tuple[Tensor]: The output features from the specified layers.
        """
        B, C, H, W = x.shape
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # 如果当前block的输出需要收集，存储到outs列表
            if i in self.out_indices:
                # 调整特征图形状，收集当前层的输出
                x = x.reshape(B, Hp, Wp, *x.shape[-3:]).permute(
                    0, 5, 1, 3, 2, 4).reshape(B, -1, Hp * x.shape[-3], Wp * x.shape[-2]).contiguous()
