...
class Bottleneck(BaseModule):
    """Bottleneck block for Re_ResFormer.
...
class ResLayer(nn.Sequential):
    """ResLayer to build Re_ResFormer style backbone.
...
class Re_ResFormer(BaseBackbone):
    """Re_ResFormer backbone.
    ...
    Example:
        >>> from mmpretrain.models import Re_ResFormer
        >>> import torch
        >>> self = Re_ResFormer(depth=18)
        >>> self.eval()
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.0):
        super(Re_ResFormer, self).__init__(init_cfg)
        if depth not in self.arch_settings:
            raise KeyError(f'invalid depth {depth} for Re_ResFormer')
        self.depth = depth
    ...
    def init_weights(self):
        super(Re_ResFormer, self).init_weights()
    ...
    def train(self, mode=True):
        super(Re_ResFormer, self).train(mode)
        self._freeze_stages()
    ...
    def get_layer_depth(self, param_name: str, prefix: str = ''):
        """Get the layer id to set the different learning rates for Re_ResFormer.
...
@MODELS.register_module()
class Re_ResFormerV1c(Re_ResFormer):
    """Re_ResFormerV1c backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default Re_ResFormer(Re_ResFormerV1b), Re_ResFormerV1c replaces the 7x7 conv
    in the input stem with three 3x3 convs.
    """

    def __init__(self, **kwargs):
        super(Re_ResFormerV1c, self).__init__(
            deep_stem=True, avg_down=False, **kwargs)


@MODELS.register_module()
class Re_ResFormerV1d(Re_ResFormer):
    """Re_ResFormerV1d backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default Re_ResFormer(Re_ResFormerV1b), Re_ResFormerV1d replaces the 7x7 conv in
    the input stem with three 3x3 convs. And in the downsampling block, a 2x2
    avg_pool with stride 2 is added before conv, whose stride is changed to 1.
    """

    def __init__(self, **kwargs):
        super(Re_ResFormerV1d, self).__init__(
