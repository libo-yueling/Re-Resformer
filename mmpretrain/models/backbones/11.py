...
# 五个部分：DSConv1 -> SN1 -> DSConv2 -> SN2 -> KAN
class ResNetEEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super(ResNetEEncoder, self).__init__()
        # 第一部分
...
class ResLayer(nn.Sequential):
    """ResLayer to build ResNetE style backbone.
...
class ResNetE(BaseBackbone):
    ...
    Example:
        >>> from mmpretrain.models import ResNetE
        >>> import torch
        >>> self = ResNetE(depth=18)
        >>> self.eval()
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.0):
        super(ResNetE, self).__init__(init_cfg)
        if depth not in self.arch_settings:
            raise KeyError(f'invalid depth {depth} for ResNetE')
        self.depth = depth
        ...
        if depth in [18, 34]:
            # self.layer1 = ResNetEEncoder(in_channels=stem_channels, hidden_channels=stem_channels, out_channels=64)
            self.layer2 = ResNetEEncoder(in_channels=64, hidden_channels=64, out_channels=128)
            # self.layer3 = ResNetEEncoder(in_channels=128, hidden_channels=128, out_channels=256)
            self.layer4 = ResNetEEncoder(in_channels=256, hidden_channels=256, out_channels=512)
        elif depth in [50, 101, 152]:
            # self.layer1 = ResNetEEncoder(in_channels=stem_channels, hidden_channels=stem_channels, out_channels=256)
            self.layer2 = ResNetEEncoder(in_channels=256, hidden_channels=256, out_channels=512)
            # self.layer3 = ResNetEEncoder(in_channels=512, hidden_channels=512, out_channels=1024)
            self.layer4 = ResNetEEncoder(in_channels=1024, hidden_channels=1024, out_channels=2048)
    ...
    def init_weights(self):
        super(ResNetE, self).init_weights()
    ...
    def get_layer_depth(self, param_name: str, prefix: str = ''):
        """Get the layer id to set the different learning rates for ResNetE.
...
@MODELS.register_module()
class ResNetEV1c(ResNetE):
    """ResNetEV1c backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default ResNetE(ResNetEV1b), ResNetEV1c replaces the 7x7 conv
    in the input stem with three 3x3 convs.
    """

    def __init__(self, **kwargs):
        super(ResNetEV1c, self).__init__(
            deep_stem=True, avg_down=False, **kwargs)


@MODELS.register_module()
class ResNetEV1d(ResNetE):
    """ResNetEV1d backbone.

    This variant is described in `Bag of Tricks.
    <https://arxiv.org/pdf/1812.01187.pdf>`_.

    Compared with default ResNetE(ResNetEV1b), ResNetEV1d replaces the 7x7 conv in
    the input stem with three 3x3 convs. And in the downsampling block, a 2x2
    avg_pool with stride 2 is added before conv, whose stride is changed to 1.
    """

    def __init__(self, **kwargs):
        super(ResNetEV1d, self).__init__(
