...
class ECAAttention1(nn.Module):
    ...
    def forward(self, x):
        y=self.gap(x) #bs,c,1,1,提取全局信息
        y=y.squeeze(-1).permute(0,2,1) #bs,1,c，去除宽，交换通道和长的位置
        y=self.conv(y) #bs,1,c，通过一维卷积自适应提取
        y=self.sigmoid(y) #bs,1,c，计算通道间权重
        y=y.permute(0,2,1).unsqueeze(-1) #bs,c,1,1，恢复到原始尺寸
        y=y.expand_as(x)
