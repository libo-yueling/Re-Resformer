...
class OOOMultiOutputHead(BaseModule):
    """Multi-Output head for classification and regression (with MLP for non-linear regression of roughness)."""
    ...
    def __init__(self,
        ...
        super(OOOMultiOutputHead, self).__init__(init_cfg=init_cfg)
        ...
        # Initialize MLP for roughness regression (non-linear regression)
        self.regressor_mlp = nn.Sequential(
            # nn.Linear(1920, 1024),  # Input layer now correctly takes 1920 features
            # nn.GELU(),  # Use GELU instead of ReLU
            # nn.LayerNorm(1024),  # Batch Normalization
            nn.Linear(960, 512),  # Second hidden layer
            nn.GELU(),
            ...
            nn.Linear(128, regressor_out_channels)  # Output layer for roughness prediction
        )
    ...
    def forward(self, feats: Tuple[torch.Tensor], data_samples: Optional[List[DataSample]] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 使用MLP进行非线性回归粗糙度
        roughness_value = self.regressor_mlp(pooled_features_for_regression)
    ...
    def loss(self, feats: Tuple[torch.Tensor], data_samples: List[DataSample], **kwargs) -> dict:
        ...
        # Ensure that roughness target has the same shape as model output (batch_size, 1)
        if targets_roughness.dim() == 1:
            targets_roughness = targets_roughness.view(-1, 1)
