...
class KapHead(BaseModule):
    """Head for regression (with linear regression for roughness) using adaptive weighted fusion for 4 stage features.

    主要改动：
      1. 不再直接 concat 所有 stage 特征，而是使用 4 个可学习权重进行自适应加权融合。
      2. 提供辅助方法 `get_stage_features`，返回每个 stage 的 flatten 特征，
         便于单独测试各 stage 特征的效果。
    """
    ...
    def __init__(self,
        ...
        super(KapHead, self).__init__(init_cfg=init_cfg)
        self.in_channels = in_channels
        self.roughness_loss_module = MODELS.build(roughness_loss)

        # 假设输入feat有4个stage的特征
        # 定义4个可学习的标量权重，用于自适应加权融合
        self.stage_weights = nn.Parameter(torch.ones(4))

        # regressor 输入特征数需要根据每个 stage flatten 后的维度之和
        # 此处假设每个 stage flatten 后的维度均为某个固定值，比如 dim_stage，
        # 则融合后总输入维度 = dim_stage（因为融合是加权求和而非拼接）
        self.regressor_after_pooling = KANLayer(960, regressor_out_channels)

    def get_stage_features(self, feats: Tuple[torch.Tensor]) -> List[torch.Tensor]:
        """将每个stage的特征单独flatten后返回，便于单独测试"""
        # 假设 feats 为4个 stage 的特征
        stage_features = [feat.view(feat.size(0), -1) for feat in feats]
        return stage_features

    def forward(self, feats: Tuple[torch.Tensor], data_samples: Optional[List[DataSample]] = None) -> torch.Tensor:
        # 提取每个 stage 的 flatten 特征
        stage_features = self.get_stage_features(feats)  # list of 4 tensors, each of shape (B, D_i)

        # 计算每个 stage 的权重（通过 softmax 保证归一化）
        weights = F.softmax(self.stage_weights, dim=0)  # shape (4,)

        # 进行自适应加权融合
        # 为简化，这里假设各 stage 的 flatten 维度相同（或经过适当处理后相同），
        # 否则需要先用线性层映射到同一维度。
        fused_features = sum(w * f for w, f in zip(weights, stage_features))

        # 这里 fused_features 的维度为 (B, D) ，D 需要与 regressor_after_pooling 输入匹配
        # 你可以根据实际情况调整 regressor_after_pooling 的输入维度
        roughness_value = self.regressor_after_pooling(fused_features)
    ...
    def predict(self, feats: Tuple[torch.Tensor], data_samples: Optional[List[Optional[DataSample]]] = None) -> List[
        ...
        # 调试时输出每个样本各 stage 的特征（如需要可自行调用 get_stage_features）
