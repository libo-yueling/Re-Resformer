...
from .utils import LABEL_TYPE, SCORE_TYPE, format_label, format_score,format_roughness
...
class DataSample(BaseDataElement):
    ...
    Data fields:
        ...
        gt_roughness (tensor): The ground truth roughness value.
        pred_roughness (tensor): The predicted roughness value.
    ...
    def set_pred_score(self, value: SCORE_TYPE) -> 'DataSample':
        """Set ``pred_score``."""
        score = format_score(value)
    ...
    def set_mask(self, value: Union[torch.Tensor, np.ndarray]) -> 'DataSample':
        """Set ``mask``."""
    ...
    def set_gt_roughness(self, value: float) -> 'DataSample':
        """Set ``gt_roughness``."""
        self.set_field(format_roughness(value), 'gt_roughness')
        return self

    def set_pred_roughness(self, value: float) -> 'DataSample':
        """Set ``pred_roughness``."""
        self.set_field(format_roughness(value), 'pred_roughness')
        return self
    ...
    def set_ori_img(self, value: torch.Tensor) -> 'DataSample':
        """Set original image tensor."""
        self.set_field(value, 'ori_img', dtype=torch.Tensor)
        return self
