...
ROUGHNESS_TYPE = Union[torch.Tensor, np.ndarray, Sequence, float]
...
#粗糙度值的处理
def format_roughness(value: ROUGHNESS_TYPE) -> torch.Tensor:
    """Convert various python types to roughness-format tensor.

    Supported types are: :class:`numpy.ndarray`, :class:`torch.Tensor`,
    :class:`Sequence`, :class:`float`.

    Args:
        value (torch.Tensor | numpy.ndarray | Sequence | float): Roughness value.

    Returns:
        :obj:`torch.Tensor`: The formatted roughness tensor.
    """
    # Handle single number
    if isinstance(value, (torch.Tensor, np.ndarray)) and value.ndim == 0:
    ...
    if isinstance(value, np.ndarray):
        ...
        raise TypeError(f'Type {type(value)} is not an available roughness type.')
    assert value.ndim == 1, \
        f'The dims of value should be 1, but got {value.ndim}.'
